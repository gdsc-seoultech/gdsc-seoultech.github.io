...
~~그래서 따라가기 좀 버거워요.~~ 그래서 아마 안드로이드 맴버님들은 모두 눈치 채셨겠지만 세션 들어가면 급격히 우울해지는 그런 느낌이랍니다. 근데 또 뭔가 알 것 같으면 기분 좋고, 되면 기분 좋고 뭐... 그렇습니다.
...
그래서 제가 물어보고 싶은 것은... 선배들도 다 이런 감정을 모두 아시겠죠? 라는 것입니다.

암튼 뭐 그래서 그런 마인드로, 운명이라고 생각하고 나름 긍정적인 마인드 갖고 출발하는, 코딩 최약체의 안드로이드 세션 3주차 리뷰 시작하겠습니다.
# 복습
* * *  
## Screen Package로 옮기기

지난주까지 저는 하나의 __MainActivity.kt__에서만 작업을 진행해왔습니다. 하지만 이제 진짜 어플을 구성하는 작업을 해주었습니다.<br> 바로 __Package__를 만들어서 정리하는 작업입니다.
__Screens__라는 __Package__를 만들어 __MainActivity.kt`에 있는 코드들을 옮겨와 작업합니다.
...
원래는 왼쪽 화면과 같이 클론을 따는게 목적이지만 지금은 편의상 오른쪽으로 화면구성을 하였습니다.
...
이렇게 __Next 버튼__ 이 안에 텍스트가 들어감에 따라 바뀌었습니다!
* * *
## MoreScreen
...
최근에는 운이 좋게 GDSC 해결사 코어 맴버님 집에 방문할 기회가 생겨서 이런 저런 얘기를 나눴습니다.<br>

저는 그 코어님과의 만남이 참 위로가 되어 좋았습니다. 나보다 훨씬 잘하는 사람이 나보다 공부를 훨씬 많이 한다는 것에서 큰 충격을 받았습니다. 당분간은 그 기억을 연료삼아 달릴 것 같습니다.
